...
for i = 1 : 25
    X_training = rand(600, m).*2 - 1; % set of random points betwee [-1,1]
    X_testing = rand(600, m).*2 - 1;
    mu = 0.0;
    SIGMA = 0.07;
    g_Ax_training = mvnpdf(X_training * A,mu,SIGMA); %+ randn(100,1).*0.05;
    g_Ax_testing = mvnpdf(X_testing * A,mu,SIGMA);%+ randn(100,1).*0.05;
